...
SELECT pkImportStagingId
  ...
  where coalesce(pkProductId, -1) <> 16676 -- exclude RR records
...
select * from (
  ...
  union all
  select 2 as warnlvl, pkImportStagingId, TabDetailDescription, coalesce(TypeDescription, 'null')+' not set to '+coalesce(DefaultContainerDescription, 'null')+' ('+coalesce(ContainerDescription, 'null')+': '+coalesce(ProductName,'null')+')' as msg from ctetis where isnull(fkContainer_Default, -1) <> isnull(fkContainerId, -2)
  union all
