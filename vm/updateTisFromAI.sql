-- updateTisFromAI.sql
-- Updates TabX.ImportStaging using AI-classified data from TabX.ImportStagingAI.
-- Run normalizeImportStagingAI.sql first.
--
-- Columns updated:
--    fkProductId, fkContainerId, NewProductName
--    IsNewProduct, IsNewProductAlias
--    fkProductTypeId, fkProductCategoryId, fkNewProductBrandId, NewProductBrand
--
-- Notes:
--    IsNewBrand (per requirements) does not exist in TabX.ImportStaging schema.
--    Add that column to the table before including it here.
--
--    Common.Containers schema was not available at script creation time.
--    Container resolution falls back to product-type defaults (see fkContainerId CASE).
--    To add a proper container lookup, join Common.Containers on ContainerType from
--    isai_dedup and use the result instead of the hardcoded IDs.
--
--    Mixed drink routing:
--       ProductType = 'Mixed Drink' AND IsWellKnownMixedDrink = 1
--          → Generic brand (pkBrandId=2); no NewProductBrand text override
--       ProductType = 'Mixed Drink' AND IsWellKnownMixedDrink = 0 or NULL
--          → Location name as NewProductBrand, fkNewProductBrandId = NULL
--
--    The intentional ProductType/ProductCategory column swap in ImportStagingAI
--    is preserved throughout. See CLAUDE.md for full explanation.
--
--    ImportStaging ↔ ImportStagingAI are joined via pkImportStagingId = fkImportStagingId
--    (FK columns) rather than text-match, eliminating duplicate rows when multiple
--    AI rows share the same ItemAsListed text for different ImportStaging rows.
--
--use TabX
--GO
...
isai_dedup as (
   select
     ...
     ,max(ProductKeywords)       as ProductKeywords          -- pipe-delimited distinguishing keywords
...
md_wellknown as (
   ...
   -- product search restricted to Mixed Drink type
   outer apply (
      select top 1 p.pkProductId
      from Common.Products as p
...
-- fkRecommendedBrandId is NULL; the location name is resolved in the
-- main UPDATE via tis.fkLocationId → TabX.Locations.Name.
-- ============================================================
md_unknown as (
   select
      isai.fkImportStagingId
     ...
     ,isai.ItemAsListed
     ,isai.ProductName
     ,cast(null as varchar(100))   as BrandResolved       -- filled by loc.Name in main UPDATE
   ...
   -- (fkProductId=12257 fallback, NewProductBrand=loc.Name, IsNewProduct=1).
   select
      isai_d.fkImportStagingId
     ,cast(null as int)            as pkImportStagingAIId
     ,isai_d.ItemAsListed
...
-- Each branch's WHERE clause is mutually exclusive, so at most
-- one branch produces a row for any given fkImportStagingId.
-- ============================================================
all_matches as (
   select
...
-- ============================================================
-- Main UPDATE
-- Joins ImportStaging to the deduplicated AI data (isai_dedup) for
-- type/category resolution, and to all_matches (ss) for brand/product.
-- Both joins use FK columns to guarantee at most one AI row per
-- ImportStaging row, eliminating duplicates.
-- ============================================================
update tis
set
   ...
   -- --------------------------------------------------------
   -- fkProductId
   -- Special product types get forced product IDs regardless
   -- of any AI match.  The Mixed Drink fallback is Generic
   -- Mixed Drink (12257).  All other unmatched rows get ZZ
   -- Undefined (1).
   -- --------------------------------------------------------
   fkProductId =
   case
      when cpt.pkProductTypeId = 6  then 12241  -- Food        → Generic Food
      when cpt.pkProductTypeId = 5  then 1       -- Undefined   → ZZ Undefined
      when cpt.pkProductTypeId = 7  then 1       -- Misc        → ZZ Undefined
      when cpt.pkProductTypeId = 11 then 1       -- Merch       → ZZ Undefined (update ID when Generic Merch product exists)
      when cpt.pkProductTypeId = 4
        or lower(trim(isai_d.ProductType)) = 'mixed drink'
         then coalesce(ss.fkRecommendedProductId, 12257)  -- Mixed Drink → matched or Generic Mixed Drink
      ...
      when ss.fkRecommendedBrandId  is null then 1   -- no brand, no product → ZZ Undefined
      else cbr.fkDefaultProductId  -- brand match, no product → brand default
   end

   -- --------------------------------------------------------
   -- fkContainerId
   -- Defaults by product type; beer and wine also check keywords
   -- in TabDetailDescription to distinguish draft/can/bottle/glass.
   -- Update this section when Common.Containers schema is available
   -- by joining on isai_d.ContainerType.
   -- --------------------------------------------------------
  ,fkContainerId =
   case
      when cpt.pkProductTypeId = 1  then   -- Beer: refine by serving descriptor
         case
            ...
            when lower(tis.TabDetailDescription) like '%can%'   then 27  -- can
            else 12                                                        -- default: 12oz draft
         end
      when cpt.pkProductTypeId = 2  then   -- Liquor: shot vs double
         case
            when lower(tis.TabDetailDescription) like '%dbl%'
              or lower(tis.TabDetailDescription) like '%double%' then 30  -- double shot
            else 26                                                        -- 1 shot
         end
      when cpt.pkProductTypeId = 3  then   -- Wine: glass vs bottle
         case
            when lower(tis.TabDetailDescription) like '%bottle%'
              or ' ' + lower(tis.TabDetailDescription) + ' ' like '% bt %'
              or ' ' + lower(tis.TabDetailDescription) + ' ' like '% btl %' then 44  -- bottle
            else 33                                                        -- 5oz wine glass
         end
      when cpt.pkProductTypeId = 4  then 35    -- Mixed Drink  → 1 drink
      when cpt.pkProductTypeId = 5  then 29    -- Undefined    → undefined
      when cpt.pkProductTypeId = 6  then 32    -- Food         → each
      when cpt.pkProductTypeId = 7  then 29    -- Misc         → undefined
      when cpt.pkProductTypeId = 8  then 40    -- Energy Drink → 8.4oz can
      when cpt.pkProductTypeId = 9  then 35    -- Non-Alcoholic → 1 drink
      when cpt.pkProductTypeId = 10 then 35    -- Mixers       → 1 drink
      when cpt.pkProductTypeId = 11 then 32    -- Merch        → each
      when cpt.pkProductTypeId = 12 then 14    -- Water        → 12oz bottle
      when cpt.pkProductTypeId = 13 then 28    -- RTD/Seltzer  → 12oz can
      when cpt.pkProductTypeId = 14 then 32    -- Garnish      → each
      else 29                                  -- fallback     → undefined
   end

   -- --------------------------------------------------------
   -- NewProductName
   -- Fixed canonical name for special product types; all others
   -- use the AI ProductName from ImportStagingAI directly.
   -- --------------------------------------------------------
  ,NewProductName =
   case
   ...
   -- --------------------------------------------------------
   -- IsNewProduct
   -- 1 if the item is highly likely to be absent from Common.Products:
   --   - no product ID was resolved at all, OR
   --   - the resolved product differs from the brand default AND
   --     the computed display name doesn't match the catalog name
   --     (suggests the AI found something the catalog doesn't have).
   -- Fixed-type items (Food/Misc/Undefined/Merch) are never new.
   -- --------------------------------------------------------
  ,IsNewProduct =
   case
   ...
   -- --------------------------------------------------------
   -- IsNewProductAlias
   -- 1 if the product exists in Common.Products but the AI-supplied
   -- name differs from the catalog name (i.e. it is a known alias).
   -- Fixed-type items are excluded.
   -- --------------------------------------------------------
  ,IsNewProductAlias =
   case
  ...
  ,fkNewProductBrandId = ss.fkRecommendedBrandId  -- NULL for unknown mixed drinks and unmatched brands
...
-- Pull resolved brand record for fkDefaultProductId used in IsNewProduct check
left outer join Common.Brands as cbr
...
-- Only update rows where AI data exists; skip ImportStaging rows not yet processed by the AI.
where isai_d.fkImportStagingId is not null

--select tis.pkImportStagingId, tis.TabDetailDescription
--      ,isai_d.ProductCategory, isai_d.ProductType
--      ,cpt.pkProductTypeId, cpt.TypeDescription
--      ,ss.BrandResolved, ss.ProductName, ss.fkRecommendedBrandId, ss.fkRecommendedProductId
--      ,ss.InfProdName
--from TabX.ImportStaging as tis
--left outer join isai_dedup as isai_d  on isai_d.fkImportStagingId = tis.pkImportStagingId
--left outer join all_matches as ss     on ss.fkImportStagingId = tis.pkImportStagingId
--left outer join Common.ProductTypes as cpt on (lower(trim(cpt.TypeDescription)) = lower(trim(isai_d.ProductCategory)) and cpt.IsActive = 1)
--left outer join Common.ProductCategories as cpc on (lower(trim(cpc.CategoryName)) = lower(trim(isai_d.ProductType)) and cpc.IsActive = 1)
--left outer join Common.Brands as cbr  on (cbr.pkBrandId = ss.fkRecommendedBrandId and cbr.IsActive = 1)
