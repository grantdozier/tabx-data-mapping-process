...
,isnull(resolved_stateprov_id, 1) as fkDefaultOriginStateProvinceId
...
(
    ...
    (
        ...
        (
            select
            max(brand) as brand
            ,lower(trim(max(brand))) as brand_norm
            ,replace(replace(replace(replace(lower(trim(max(brand))),' ',''),'''',''),'-',''),'.','') as brand_norm_s
            ,max(ItemAsListed) as ItemAsListed
            ,max(City) as City
            ,max(StateProv) as StateProv
            ,max(Country) as Country
            ,max(lower(trim(Country))) as country_norm
            ,max(upper(trim(CountryCode))) as country_code
            ,max(BrandKeywords) as brand_keywords
            ,max(lower(trim(StateProv))) as stateprov_norm
            from TabX.ImportStagingAI
            where brand is not null
            group by lower(trim(brand))
        ) as isai
        left outer join common.Brands as cb   on (lower(trim(cb.BrandDescription)) = isai.brand_norm and cb.IsActive = 1)
        left outer join common.Brands as cbs  on (lower(trim(cbs.BrandDescriptionShort)) = isai.brand_norm and cbs.IsActive = 1)
        left outer join common.Brands as sinb on (lower(trim(sinb.BrandDescription)) like '%' + replace(replace(replace(replace(isai.brand_norm,'\','\\'),'%','\%'),'_','\_'),'[','\[') + '%' escape '\' and sinb.IsActive = 1)
        left outer join common.Brands as bins   on (isai.brand_norm like '%' + replace(replace(replace(replace(lower(trim(bins.BrandDescription)),'\','\\'),'%','\%'),'_','\_'),'[','\[') + '%' escape '\' and bins.IsActive = 1)
        left outer join common.Brands as cb_s   on (replace(replace(replace(replace(lower(trim(cb_s.BrandDescription)),' ',''),'''',''),'-',''),'.','') = isai.brand_norm_s and cb_s.IsActive = 1)
        left outer join common.Brands as bins_s on (isai.brand_norm_s like '%' + replace(replace(replace(replace(replace(replace(replace(replace(lower(trim(bins_s.BrandDescriptionShort)),'\','\\'),'%','\%'),'_','\_'),'[','\['),' ',''),'''',''),'-',''),'.','') + '%' escape '\' and len(bins_s.BrandDescriptionShort) >= 5 and bins_s.IsActive = 1)
        group by Brand
    ) agg
    outer apply (
        select top 1 cba.BrandAlias
        from Common.BrandAliases as cba
        where lower(trim(cba.BrandAlias)) = agg.brand_norm
          and cba.IsActive = 1
    ...
    outer apply (
        ...
        where cc_code.pkCountryId is null
          and (   agg.country_norm = lower(trim(cc.CountryName))
               or agg.country_norm = lower(trim(cc.CountryCode))
               or agg.country_norm = lower(trim(cc.CountryCommonName)))
    ...
    outer apply (
        ...
        where cc_code.pkCountryId is null
          ...
          and cca.IsActive = 1
          and agg.country_norm = lower(trim(cca.CountryAlias))
        order by cca.pkCountryAliasId asc
    ) as cca
    outer apply (
        select top 1 pkStateProvinceId
        from Common.StateProvinces
        where agg.stateprov_norm = lower(trim(StateProvinceName))
           or agg.stateprov_norm = lower(trim(Abbrev))
        order by pkStateProvinceId asc
    ) as csp
    outer apply (
        select top 1 fkStateProvinceId
        from Common.StateProvinceAliases
        where agg.stateprov_norm = lower(trim(StateProvinceAlias))
